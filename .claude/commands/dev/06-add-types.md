---
description: packages/shared に型定義・Zodバリデーションスキーマを追加する。機能実装前に必ず実行する。
...
`packages/shared` に TypeScript 型定義と Zod バリデーションスキーマを追加する。
...
3. `packages/shared/src/types/` — 既存の型定義（重複チェック）
4. `packages/shared/src/validations/` — 既存のスキーマ（重複チェック）
...
- **引数あり**: 指定された機能名を対象にする
- **引数なし**: `docs/requirements/architecture.md` のデータモデルセクションから全機能を抽出
...
`packages/shared/src/types/{feature}.ts` に以下のパターンで定義:
...
`packages/shared/src/validations/{feature}.ts`:
...
// Validations
export { create{Feature}Schema, update{Feature}Schema } from './validations/{feature}'
export type { Create{Feature}Input, Update{Feature}Input } from './validations/{feature}'
```
...
- [ ] `packages/shared/src/types/{feature}.ts` が作成されている
- [ ] `packages/shared/src/validations/{feature}.ts` が作成されている
- [ ] `packages/shared/src/index.ts` に export が追加されている
- [ ] Web・Expo どちらも `@resave/shared` からimportできる
