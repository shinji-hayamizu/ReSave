...
各フェーズ完了時に自動停止し、`/clear` 実行を促す。

```
/dev/cc-sdd api-backend-generate --chunked
```

**動作**: 1フェーズ実行 → 停止 → clear促し → 再開コマンド表示
...
コンテキスト節約のため、/clear を実行してから次のフェーズに進むことを推奨します。

再開コマンド:
/dev/cc-sdd {spec-name} --from Y.1
---
```
...
- `--chunked` で大規模タスクを分割
- **各フェーズ完了時に `/clear` を実行**してから次フェーズへ
- 状態ファイルで進捗を永続化
- clearしても再開コマンドで続行可能
...
```
1. /dev/cc-sdd spec-name --chunked
2. Phase 1 完了 → /clear 実行
3. /dev/cc-sdd spec-name --from 2.1
4. Phase 2 完了 → /clear 実行
5. 繰り返し...
```
